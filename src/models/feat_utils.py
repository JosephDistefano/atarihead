...
def fuse_gazes(images_, gazes, gaze_count=-1):
    gazes_ = [
        torch.stack([torch.Tensor(gaze_pdf(gaze_, gaze_count)) for gaze_ in gaze_stack])
...
def fuse_gazes_noop(
    images_, gazes_, actions, gaze_count=-1, fuse_type="stack", fuse_val=1
):
    ...
    gazes_ = action_mask.to(device=torch.device("cuda")) * torch.stack(gazes_).to(
    ...
    if fuse_val == 1:
        gazes_ = torch.stack(
            [
                torch.stack(
                    [
                        torch.ones((84, 84)).to(device=torch.device("cuda"))
                        if torch.sum(g) == 0
                        else g
                        for g in gst
                    ]
        ...
        gazes_ = torch.stack(
            [
                torch.stack(
                    [
                        torch.zeros((84, 84)).to(device=torch.device("cuda"))
                        if torch.sum(g) == 0
                        else g
                        for g in gst
                    ]
