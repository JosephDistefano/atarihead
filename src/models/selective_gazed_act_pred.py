...
def train_seas_model(config_data, game, Mode="train"):
    ...
    if MODE == "eval":
        if GAZE_TYPE == "PRED":
            ...
            xg = (
                gaze_net.infer(image_)
                .repeat(1, image_.shape[1], 1, 1)
                .to(device=device)
            )
        ...
        if GAZE_TYPE == "PRED":
            ...
            action_net.train_loop(optimizer, lr_scheduler, loss_, batch_size=BATCH_SIZE)
