...
class MDN(nn.Module):
    def __init__(
        self, input_shape=(80, 80), num_gaussians=10, load_model=False, epoch=200
    ):
    ...
    def gaussian_distribution(self, y, mu, sigma, rho=0):
        ...
        assert sigma_x.shape == sigma_y.shape, print(sigma_x.shape, sigma_y.shape)
        ...
        for coord in y:
            ...
            x_val = torch.pow(cx - mu_x, 2) * torch.reciprocal(torch.pow(sigma_x, 2))
            y_val = torch.pow(cy - mu_y, 2) * torch.reciprocal(torch.pow(sigma_y, 2))
            num = torch.exp(-(x_val + y_val))
            # self.writer.add_histogram('x_val', x_val)
            # self.writer.add_histogram('y_val', y_val)

            pdf = num * torch.reciprocal(2 * np.pi * torch.mul(sigma_x, sigma_y))
...
def train_loop(net, opt, x_var, y_var, batch_size):
    ...
    for epoch in range(10000):
        pi_variable, sigma_variable, mu_variable = net(x_var)

        loss = net.loss_fn(pi_variable, sigma_variable, mu_variable, y_var)
        opt.zero_grad()
        loss.backward()
...
if __name__ == "__main__":
    ...
    #           for _ in range(42)]
    rand_y = [[np.random.random(), np.random.random()] for _ in range(42)]
