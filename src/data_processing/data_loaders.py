...
import pandas as pd
import h5py
from .data_utils import stack_data  # , transform_images
from models.feat_utils import fuse_gazes_noop, fuse_gazes
...
def load_interm_data(
    config, game="breakout", game_run="198_RZ_3877709_Dec-03-16-56-11"
):
    ...
    """
    # load directories of game runs and gaze files
    game_dir = os.path.join(config["interim_data_dir"], game)
    game_run_dir = os.path.join(game_dir, game_run)
    gaze_file = os.path.join(game_run_dir, game_run + "_gaze_data.csv")

    # turn gaze data in a pd dataframe
    gaze_data = pd.read_csv(gaze_file)

    # Goes through tall the files and gets the frame names
...
def load_images_action_gaze_data(
    config,
    stack=1,
    stack_type="",
    stacking_skip=1,
    ...
    """Loads and processes gaze data, frames, and actions for the specified game and game run
    ...
    actions_ : all actions for specified game run
    """
    gaze_data, game_run_dir = load_interm_data(config, game=game, game_run=game_run)
    gaze_range = [160.0, 210.0]  # w,h

    # normalizing the gaze data
    ...
    images_, actions_, gazes_ = stack_data(
        images,
        actions,
        gazes,
        stack=stack,
    ...
    return images_, actions_, gazes_
...
def load_data_iter(
    ...
    # print(data_types, type(data_types))
    # print(dataset, type(dataset))
    # print(dataset_exclude, type(dataset_exclude))
    ...
    if load_type == "memory":
        ...
        dataset = HDF5TorchDataset(
            game=game, data=data_types, dataset=dataset, device=device
        )
    elif load_type == "live":
        print("prepping and loading data for {},{}".format(game, dataset))
        images_, actions_ = load_action_data(
            stack=config["STACK_SIZE"], game=game, till_ix=-1, game_run=dataset
        )
...
class HDF5TorchChunkDataset(data.Dataset):
    def __init__(
        ...
        groups = list(
            sorted(set(self.hdf5_file.keys()) - set(self.dataset_exclude), reverse=True)
        )
        ...
        self.group_lens = [self.hdf5_file[g]["actions"].len() for g in groups]
    ...
    def __load_data__(self):
        self.curr_collation_data = load_hdf_data(
            game=self.game, dataset=self.curr_collation, data_types=self.data_types
        )
        ...
        for dtype in self.curr_collation_data:
            ...
            if dtype == "actions":
                datum = torch.LongTensor(datum).squeeze()[:, -1].to(device=self.device)
            else:
    ...
    def __reset_dataset__(self):
        ...
        if (
            self.curr_collation_epoch == self.num_epochs_per_collation
            or self.curr_collation is None
        ):
            self.curr_collation_epoch = 0
            ...
            if "actions" in self.curr_collation_data:
                labels = self.curr_collation_data["actions"].cpu().numpy().copy()
                label_to_count = Counter(labels)
                weights = torch.DoubleTensor(
                    [1.0 / label_to_count[ix] for ix in labels]
                )
                self.sample_ixs = torch.multinomial(
                    weights, self.curr_collation_len, replacement=True
                )
            else:
...
class HDF5TorchDataset(data.Dataset):
    ...
    def __getitem__(self, ix):
        ...
        for datum in self.data:
            if datum == "actions":
                tensors.append(torch.tensor(self.labels[ix]).to(device=self.device))
            else:
                tensors.append(
...
if __name__ == "__main__":
    ds = HDF5TorchChunkDataset(
        game="breakout", num_groups_to_collate=2, num_epochs_per_collation=1
    )
