...
game_run_data = []
with open(os.path.join(os.path.join(RAW_DATA_DIR, game), game_run) + ".txt", "r") as f:
...
for tstep in game_run_data:
    ...
    if "null" in tstep_:
        ...
        assert len(tstep_) == len(header) - 1, print(
            tstep_, header, len(tstep_), len(header)
        )
    ...
    if len(gaze_data) == 1 and gaze_data[0] == "null":
        ...
        gaze_data_ = [gaze_data[ix : ix + 2] for ix in range(0, len(gaze_data) - 1, 2)]
...
assert len(frame_ids) == len(game_run_frames), print(
    len(frame_ids), len(game_run_frames)
)
proc_writ_dir = os.path.join(os.path.join(PROC_DATA_DIR, game), game_run) + "_wgz"
...
for fid in tqdm(frame_ids.index):
    wix += 1
    assert (
        game_run_data_mod_df.iloc[fid]["frame_id"]
        == game_run_frames[fid + 1].split(".png")[0]
    )
    ...
    for ix in range(len(game_run_data_mod_df.iloc[fid]["gaze_positions"])):
        opacity = (0.1 * ix) / len(game_run_data_mod_df.iloc[fid]["gaze_positions"])
        ...
        cv2.addWeighted(
            frame_img_data_olay, opacity, frame_img_data, 1 - opacity, 0, frame_img_data
        )
        # print(gpt)
    ...
    gpts = (
        np.array(game_run_data_mod_df.iloc[fid]["gaze_positions"])
        .astype(np.float)
        .astype(np.int)
    )
    ...
    # cv2.waitKey()
    cv2.imwrite(os.path.join(img_writ_dir, game_run_frames[fid + 1]), frame_img_data)
